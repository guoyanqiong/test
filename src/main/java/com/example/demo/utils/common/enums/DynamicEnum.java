...
public class DynamicEnum {
	...
	public static <T extends Enum<?>> void updateOrAddEnum(Class<T> enumType, String enumName,
		...
		try {
			...
			T[] previousValues = (T[]) valuesField.get(enumType);
			List<T> values = new ArrayList<T>(Arrays.asList(previousValues));
			List<T> valuesNew = new ArrayList<T>();
			boolean flag = true; // 枚举中是否已存在键
			for (T item : values) {
			...
			// 5. Set new values field
			setFailsafeFieldValue(valuesField, null, valuesNew.toArray((T[]) Array.newInstance(enumType, 0)));
